    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>{{$_lang['spost_tag_texts']}} <span class="tagName text-primary">#</span> @if($sPost->langDefault() != 'base') {{$_lang['spost_on_lang']}} <span class="tagLang text-muted text-uppercase"></span> @endif</p>
                <textarea id="{{$sPost->langDefault()}}_tagContent" cols="30" rows="10"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">{{$_lang["cancel"]}}</button>
                <button data-id="" data-lang="" class="btn btn-success js_save_texts">{{$_lang['save']}}</button>
            </div>
        </div>
    </div>
</div>
